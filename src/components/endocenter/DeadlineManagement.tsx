...
export default function DeadlineManagement() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`ios-card overflow-visible relative ${openStatusDeadlineId ? "z-50" : "z-0"}`}>
              ...
              {editMode ? (
                <>
                  <div className="grid grid-cols-[1fr_80px_28px] gap-1.5">
                    ...
                    />
                    <button
                      onClick={() => { addNotification({ title: "Cenário de crise removido", description: crisis.scenario, icon: "delete" }); removeCrisisScenario(crisis.id); }}
                      className="rounded-md p-1 text-destructive hover:bg-destructive/10"
                    >
                      <Trash2 className="h-3 w-3" />
                    </button>
