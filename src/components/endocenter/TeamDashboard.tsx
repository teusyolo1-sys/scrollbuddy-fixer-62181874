...
export default function TeamDashboard() {
  ...
  const metrics = [
    ...
    { label: "Custo Médio / Hora", value: `R$ ${totalHours > 0 ? (totalRemuneration / totalHours).toFixed(2).replace(".", ",") : "0"}`, sub: "Eficiência da equipe", Icon: TrendingUp, color: "#059669" },
    { label: "Profissionais Ativos", value: String(team.filter(m => m.status === "Ativo").length), sub: "Todos operacionais", Icon: Users, color: "#DC2626" },
  ];
      ...
      <div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {team.map((member, i) => {
            ...
            return (
              <div key={member.id} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                <div className="p-5 flex items-center gap-4" style={{ borderLeft: `4px solid ${member.color}` }}>
                  ...
                  <span className="text-xs font-semibold px-2 py-1 rounded-full" style={{
                    backgroundColor: member.status === "Ativo" ? "#ECFDF5" : "#FFF1F2",
                    color: member.status === "Ativo" ? "#059669" : "#DC2626"
                  }}>
                    {member.status}
                  </span>
                </div>
          ...
          {team.map((m) => (
            <div key={m.id} className="flex items-center justify-between py-2 px-3 rounded-lg bg-slate-50">
              <div className="flex items-center gap-2">
                {m.photoUrl ? (
                  <img src={m.photoUrl} alt="" className="w-4 h-4 rounded-full object-cover" />
                ) : (
              ...
              <div className="text-right">
                ...
                <span className="text-xs text-slate-400 ml-2">{m.hours}h · R$ {m.hours > 0 ? (m.remuneration / m.hours).toFixed(2) : "0"}/h</span>
              </div>
            </div>
          ))}
        </div>
